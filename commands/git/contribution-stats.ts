...
async function getAllPRStats(
  ...
  const userPRs = await searchGitHub(
    octokit,
    `is:pr author:${user} archived:false`,
  );
  const reviewedPRs = await searchGitHub(
    octokit,
    `is:pr -author:${user} reviewed-by:${user} archived:false`,
  );

  const sinceTimestamp = since.getTime();

  const openedPRs = userPRs.filter((pr) =>
    new Date(pr.created_at).getTime() >= sinceTimestamp
  );
  ...
  const closedNotMergedPRs = userPRs.filter((pr) => {
    ...
    return closedAt && !pr.pull_request?.merged_at &&
      new Date(closedAt).getTime() >= sinceTimestamp;
  });
  const openPRs = userPRs.filter((pr) => pr.state === 'open');
  const reviewedPRsInPeriod = reviewedPRs.filter((pr) =>
    new Date(pr.updated_at).getTime() >= sinceTimestamp
  );
...
export const contributionStatsCommand = new Command()
  .description('Get contribution statistics for a GitHub user')
  .option('-u, --user <name:string>', 'GitHub username', { required: true })
  .option('-d, --days <num:number>', 'Days to look back', { default: 7 })
  .action(async ({ user, days }) => {
    ...
    try {
      ...
      console.log(
        `   - ${stats.daysWithCommits} days with commits in the last ${days} days.`,
      );
      console.log(
        `   - Avg: ${
          stats.avgDaysPerWeek.toFixed(2)
      ...
      console.log(`\n👀 Reviews:`);
      console.log(
        `   - ${stats.reviewedPRs} PRs reviewed (via comments or approvals)`,
      );
